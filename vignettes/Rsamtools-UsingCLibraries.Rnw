...
these also, be sure to do so appropriately to maximize portability:

For \verb|Makevars|, add to the respective \verb|PKG_LIBS| or
\verb|PKG_CPPFLAGS| line, e.g.,
...
For Makevars.win, use the \verb|+=| append syntax, e.g.,

\begin{verbatim}
PKG_LIBS+=-lfoo
PKG_CPPFLAGS+=-I/path/bar
\end{verbatim}
