...
In addition to its low level facilities for importing `BAM' files
into R, the \Rsamtools{} package defines the \Rclass{GappedAlignments}
class which is a high level container for storing a set of alignments.
...
First we load a toy `BAM' file into a \Rclass{GappedAlignments} object:
