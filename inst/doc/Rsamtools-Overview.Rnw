...
\Rfunction{GenomicRanges::readGAlignments}; the specified fields
...
\Biocpkg{Rsamtools}. In \Rpackage{ShortRead}, \Rfunction{readAligned} invoked
with \Rcode{type="bam"} will read \bam{} files in to an
\Rclass{AlignedRead} object. This function takes a \Rcode{param}
...
through the \Rfunction{readGAlignments} function in
...
@ 
%% 
Then we use the appropriate \Rpackage{TranscriptDb} package to
...
organisms, perhaps making a \Robject{TranscriptDb} object as outlined
