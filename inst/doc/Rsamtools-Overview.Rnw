...
<<indexed-file>>=
list.files(dirname(bamFile), pattern="ex1.bam*")
@ 
%% 
...
points to the bam file corresponding to individual NA19240 chromosome
6 Solexa (lllumina) sequences aligned using MAQ. The remote file is
very large (about 10 GB), but the corresponding index file is small
(about 500kb). With \Rcode{na19240url} set to the above address, the
following retrieves just those reads in the specified range
...
passing the filter to a new file.  The function \Rfunction{indexBam}
...
\Rfunarg{what} and \Rfunarg{where} arguments to
...
a strategy might process the file in chunks, with each chunk
...
summary function that expects a single (chromosome) sequence name,
reads in specific information from the \bam{} file, and calculates
...
to marshal \emph{references} to the data into a container, and then to
act on components of the container.  The \Rclass{BamViews} class
...
relevant \bam{} files and metadata (e.g., the phenotypic samples
corresponding to each \bam{} file) about the view.
...
humans. We'll proceed by identifying the ranges we are interested in,
and then the bam files to be queried.
...
@ 
%% 
It might make sense to annotate this instance with information
providing its provenance, e.g.,
<<features-annotations,keep.source=TRUE>>=
metadata(bamRanges) <- 
    list(description="KEGG/biomaRt caffeine metabolism transcripts",
         created=date())
@ 

\subsection{BAM files}

The following were performed at the time the vignette was written;
location of on-line resources and particular the organization of the
1000 genomes bam files may have changed.

We're interested in collecting the URLs of a number of BAM files from
the 1000 genomes project. Our first goal is to identify files that
might make for an interesting comparison.  First, let's visit the 1000
genomes ftp site and discover available files. We'll use the
...
files that exactly six \texttt{'.'} characters in their name (this
corresponds to files
...
@ 
<<slxMaq09-readlines,echo=FALSE>>=
slxMaq09 <- 
    readLines(system.file("extdata", "slxMaq09_urls.txt", package="Rsamtools"))
@ 
...
\subsection{Assembling and using a \Rclass{BamViews} instance}

We assemble the \Rclass{BamViews} instance by providing the BAM file
urls and genomic features of interest; we can also provide information
to annotated the BAM files (with the argument \Rfunarg{bamSamples}
function argument, which is a \Rclass{DataFrame} instance with each
row corresponding to a BAM file) and the instance as a whole (with
\Rfunarg{bamExperiment}, a simple \Rclass{list} containing information
structured as the user sees fit).
...
Methods in \Rpackage{Rsamtools} are designed to work with
\Rclass{BamViews} objects. For instance, here we query the 1000
genomes project for reads overlapping our transcripts; by loading the
\Rpackage{multicore} package, we tell \Rsamtools{} to use as many
cores as are available on our machine (the \Rpackage{multicore}
package is not currently -- as of 25 March, 2010 -- reliable on
Windows computers)
...
@ 
%% 
The resulting object is about 6 MB in size; here we load it from disk
%% 
<<olaps>>=
...
has a transcript id
\Sexpr{elementMetadata(bamRanges(bv))[["tx_name"]][1]}. Here we
...
rng <- bamRanges(bv)[1]
strand(rng) <- "*"
olap1 <- endoapply(olaps, function(elt, rng) {
    m <- findOverlaps(elt, rng)
    elt[sort(unique(queryHits(m)))]
}, rng)
olap1
head(olap1[[24]])
@ 
