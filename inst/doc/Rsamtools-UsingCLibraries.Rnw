...
This document is meant for package developers wanting to usethe
...
following code in \verb|src/Makevars|:
\begin{verbatim}
SAMVARS=$(shell \
  echo 'cat(system.file(sprintf("%s%s", "usretc", .Platform[["r_arch"]]),\
              "Rsamtools.mk", package="Rsamtools", mustWork=TRUE))' |\
      "${R_HOME}/bin/R" --vanilla --slave)
include $(SAMVARS)
\end{verbatim}
This updates the environment variables \verb|$PKG_CPPFLAGS| and
\verb|$PKG_LIBS|; if your \verb|Makefile| modies these also, be sure
