...
SEXP idxstats_bamfile(SEXP ext)
{
    ...
    name = Rf_allocVector(STRSXP, n + 1); SET_VECTOR_ELT(result, 0, name);
    len  = Rf_allocVector(INTSXP, n + 1); SET_VECTOR_ELT(result, 1, len);
    map = Rf_allocVector(REALSXP, n + 1); SET_VECTOR_ELT(result, 2, map);
    unmap = Rf_allocVector(REALSXP, n + 1); SET_VECTOR_ELT(result, 3, unmap);
    ...
    INTEGER(len)[n] = 0;
    REAL(map)[n] = counts[-1][0];
    REAL(unmap)[n] = counts[-1][1];
