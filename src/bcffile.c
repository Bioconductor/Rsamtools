...
SEXP
bcffile_open(SEXP filename, SEXP indexname, SEXP filemode)
{
...
int
scan_bcf_range(bcf_t *bcf, bcf_hdr_t *hdr, SEXP ans,
               int tid, int start, int end, int n)
{
    ...
    while (0 <= (res = vcf_read(bcf, hdr, bcf1))) {
        if (tid >= 0) {
            ...
            if (bcf1->tid != tid || bcf1->pos > end) break;
            if (!(pos >= start && end > bcf1->pos)) continue;
        }
        if (n >= sz)
...
SEXP
scan_bcf(SEXP ext, SEXP space, SEXP tmpl)
{
    ...
    if (R_NilValue == space) {
        ...
        for (int i = 0; i < nspc; ++i)
        {
            ...
            if (tid < 0) {
                ...
                Rf_error("'space' not in file: %s",
                         CHAR(STRING_ELT(spc, i)));
            }
